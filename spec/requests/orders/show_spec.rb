...
RSpec.describe "Orders", type: :request do
  ...
  before do
    ...
    create(:product, :books, name: "Flow", price: 15.50, users_score: 2).tap do |product|
      create(:order, :ordered, order_date:, user:, product:, units: 3)
    ...
    create(:product, :books, name: "Alice in Wonderland", price: 9.99, users_score: 4).tap do |product|
      create(:order, :ordered, order_date:, user:, product:, units: 2)
    end
  ...
  describe "GET orders#show - orders ordered" do
    before do
      auth_headers(user)
      get "/users/#{user.id}/orders/#{order_date}/products"
    end
    ...
    it "responds with order details" do
      ...
      expect(result[0]["name"]).to eq("Flow")
      expect(result[0]["units"]).to eq(3)
      expect(result[0]["unit_price"].to_f).to eq(15.5)
      expect(result[0]["total"].to_f).to eq(3 * 15.5)

      expect(result[1]["name"]).to eq("Alice in Wonderland")
      expect(result[1]["units"]).to eq(2)
      expect(result[1]["unit_price"].to_f).to eq(9.99)
      expect(result[1]["total"].to_f).to eq(2 * 9.99)
    end
    ...
    context "as user is not authorized to see the others user orders" do
      ...
      it "responds with error" do
        expect {
          get "/users/#{other_user.id}/orders/#{order_date}/products"
  ...
  describe "GET orders#show - shopping cart" do
    before do
      auth_headers(user)
      get "/users/#{user.id}/shopping_cart"
    end
    ...
    it "responds with order details" do
      ...
      expect(result[0]["name"]).to eq("The Godfather 2")
      expect(result[0]["units"]).to eq(3)
      expect(result[0]["unit_price"].to_f).to eq(21.50)
      expect(result[0]["total"].to_f).to eq(3 * 21.5)
    end
    ...
    context "as user is not authorized to see the others user orders" do
      ...
      it "responds with error" do
        expect {
          get "/users/#{other_user.id}/shopping_cart"
        }.to raise_error Pundit::NotAuthorizedError
      end
    end
